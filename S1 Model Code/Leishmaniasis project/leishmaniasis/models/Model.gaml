/**
* Name: Finalbatch
* Tags: Leishmania, Rodent, Human, Sandfly, Transmission 
*/ model Finalbatch
...
global {
 ...
 // parameters relative to sandfly 
 float infectionFactor<-0.1;// a constant
 float mosquitoDeath<-0.002;//1/(3*7*24), sandfly death rate
 ...
 int
	...
	float MIN_FOOD_DETECTION <- 0.2; // the minimum amount of resource that a rodent can see, 
...
//  list<float> movementProb; // list of the probability of moving to another cell
 ...
 float food <- grid_value min: 0.0 max:
	...
	float h <- 100.0;
	float t; // time 
 float x <- 4.0; // sandfly initial number in each cell
...
float max_K<-500.0;
 ...
 float ratio update: empty(RodentHereList) ? 0 : infectedHere / length(RodentHereList);
  ...
  //  diff(IM,t) =  infectionFactor * ratio * (x - IM);
	...
	float IM_stepPlusOne <- 0.0; 
	
	//Mosquitoes(sandfly) Dynamic
 reflex Verhulst {
		x_stepPlusOne <- C_mosquito * pp * x * (1 - x / (k)) + x;
		//pp : growth rate dep on temp and ndvi, (27temp, max indvi ) max number of mosquito in the scale carrying capacity in the cell 

		x <- x_stepPlusOne;
	} //Infected Mosquitoes(sandfly)

	reflex MosquitoEpidemie {
		IM_stepPlusOne <- infectionFactor * ratio * (x - IM) - mosquitoDeath*IM + IM;
		IM <- IM_stepPlusOne; 
		//      write " IM " + self.IM +  " ratio" + ratio + "   x   " + self.x + "  old IM"  + self.IM  ;
 }
...
experiment simulation type: gui {
	...
	parameter "infectionFactor" var:infectionFactor <-0.55;//fit sandfly infectionFactor to sandfly ratio data
...
// monitor "Total distance" value: total_distance;
 ...
 //main display shows rodent agent dynamic       
 ...
 //           species roden aspect: base;
		...
		display humanDynamic refresh: every(1 #day) {
		...
		display ratios refresh: every(5 #cycle) {
			chart "Disease spreading ratios" type: series size: {1, 0.5} position: {0, 0} {
				data "sandfly ratio" value: sum(terrain_cell collect (each.IM)) / sum(terrain_cell collect (each.x)) color: #red;
				data "rodent ratio " value: roden count (each.isInfected = true) / length(roden) color: #orange;
				data "human ratio" value: IH/N color:#green;
				}
			}
			
			display sandflyDynamic refresh: every(5 #cycle){
				chart "sandfly Disease number" type: series size: {1, 0.5} position: {0, 0} {
			      data "total sandfly" value: sum(terrain_cell collect (each.x)) color:#blue;
			      data "infected sandfly" value: sum(terrain_cell collect (each.IM)) color:#red;
				}
			}
}
